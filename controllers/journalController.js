...
exports.getTrialBalance = async (req, res, next) => {
  try {
    ...
    const report = Object.values(accountBalances).map(account => {
      ...
      account.debit = parseFloat(account.debit.toFixed(2));
      account.credit = parseFloat(account.credit.toFixed(2));
    ...
    report.forEach(account => {
      ...
      byType[account.type].accounts.push(account);
      byType[account.type].totalDebit += account.debit;
      byType[account.type].totalCredit += account.credit;
    });
